    ...
    def save_comprehensive_stats(self, df: pd.DataFrame, status: dict, output_path: str):
        ...
        stats = {
            "目标设定": {
                ...
                "target_inactive": self.TARGET_INACTIVE,
                "target_ratio": f"1:{self.TARGET_RATIO}",
                "ic50_active_threshold_um": self.THRESHOLDS['IC50']['active'],
                "ec50_active_threshold_um": self.THRESHOLDS['EC50']['active'],
                "inactive_threshold_um": self.THRESHOLDS['IC50']['inactive']
            },
            "最终结果": {
                "total_compounds": int(len(df)),
                "active_compounds": int((df['label'] == 1).sum()),
                "inactive_compounds": int((df['label'] == 0).sum()),
                "actual_ratio": f"1:{(df['label'] == 0).sum() / max((df['label'] == 1).sum(), 1):.2f}",
